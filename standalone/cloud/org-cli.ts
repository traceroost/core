/**
 * `traceroost org <link|status|leave>` — the CLI half of AL 01.
 *
 * Pure orchestration over `src/cloud/org/`. Prints the exact same SENT / NEVER_SENT list the Org
 * panel and the OAuth consent screen show, then runs the PKCE flow (or the `--device` fallback).
 */

import { linkInteractive, linkViaDevice, leave } from '../../src/cloud/org/link'
import { getOrgStatus } from '../../src/cloud/org/status'
import { getQueueStats } from '../../src/cloud/forward/currentQueueStats'
import { SENT, NEVER_SENT, whoSeesWhat, LEAVE_HINT } from '../../src/cloud/org/privacy'
...
async function runLink(args: string[]): Promise<number> {
  if (loadCredentials()) {
    console.log('This machine is already linked. Run `traceroost org leave` first to re-link.')
    return 1
  }
  const device = args.includes('--device')
  console.log(`Linking this machine to TraceRoost Cloud at ${orgEndpoint()}.`)
  printPromise()
...
function runStatus(): number {
  const status = getOrgStatus(getQueueStats())
  if (!status.linked) {
    console.log('Not linked. TraceRoost is working locally and sending nothing anywhere.')
    console.log('Run `traceroost org link` to join an org.')
    return 0
  }
...
/**
 * `traceroost org verify` — the one network call `status` deliberately never makes (see the
 * doc comment on getOrgStatus: local-only, by design), for the question status can't answer:
...
async function runVerify(): Promise<number> {
  ...
  if (!creds) {
    console.log('Not linked — nothing to verify. Run `traceroost org link` to join an org.')
    return 0
  }
  ...
  console.log(
    `\n${gap} trace${gap === 1 ? '' : 's'} local but not (yet) in cloud. If you were mid-session ` +
      ...
      'eligible. If the gap stays the same size a few minutes from now, run `traceroost org ' +
...
export async function runOrgCli(args: string[]): Promise<number> {
  const sub = args[0]
  switch (sub) {
